...
import { resolveDateValue, parseDateExpr } from '../date-utils';
...
export class SemanticAnalysisStage implements CompilerStage {
  ...
  private validateParameterType(value: unknown, expectedType: string): boolean {
    switch (expectedType) {
      ...
      case 'date':
      case 'datetime':
        // After coercion the value is a resolved date string; reject unparseable values.
  ...
  private coerceParameterValue(value: unknown, type: string): unknown {
    ...
    switch (type) {
      ...
      case 'date':
        // Resolve relative expressions (now, now-7d, ...) / ISO literals at compile time.
