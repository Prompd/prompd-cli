...
import { resolvePackageFile } from '../package-ref';
...
export class DependencyResolutionStage implements CompilerStage {
   ...
   */
  private async resolvePackage(context: CompilationContext, packageRef: string): Promise<string> {
    if (!context.packageResolver) {
      throw new Error(
        `Package resolution is unavailable in this environment: ${packageRef}. ` +
        `Compile via the backend to resolve packages and inheritance.`
      );
    }
  ...
  private async resolvePackageImport(
    ...
    try {
      // Resolve via the injected package resolver (Node CLI only).
  ...
  private async processInheritsField(
    ...
    if (
      ...
      resolvedInheritsRef.startsWith('../') ||
      resolvedInheritsRef.startsWith('/') ||
      (!resolvedInheritsRef.startsWith('@') && isPrompdFile(resolvedInheritsRef))
    ) {
      ...
      try {
        ...
        if (match) {
          ...
          // Resolve the package to its local path via the injected resolver.
          const packagePath = await this.resolvePackage(context, packageRef);
