...
import { extname, resolvePosix, basenamePosix, isPrompdFile } from '../path-utils';
import { resolveDateValue } from '../date-utils';
    ...
    try {
      ...
      for (const ext of possibleExtensions) {
        ...
        if (await fs.exists(filePath)) {
          ...
          // If it's a Prompd source (.prmd/.md), parse and extract content
  ...
  private async processInheritance(context: CompilationContext, content: string): Promise<string> {
    ...
    try {
      ...
      if (isPrompdFile(parentPath) && await fs.exists(parentPath)) {
        ...
        // Package directory - find main Prompd source file (.prmd or .md)
        const files = (await fs.readdir(parentPath)).filter((f: string) => isPrompdFile(f));

        if (files.length > 0) {
          // Look for main.prmd / main.md first
          const mainFile = files.find((f: string) => f === 'main.prmd' || f === 'main.md');
