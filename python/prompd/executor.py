...
from prompd.config import ParameterManager, PrompdConfig
from prompd.exceptions import PrompdError, ProviderError, SubstitutionError
from prompd.models import ExecutionContext, LLMResponse, PrompdFile
from prompd.parser import PrompdParser
from prompd.providers import ProviderConfig, registry
...
class PrompdExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            # Built-in providers (openai, anthropic) map directly; custom providers
            # use their configured type (e.g. "openai-compatible" -> "openai")
            ...
            if format_provider not in ("openai", "anthropic"):
                ...
                try:
                    ...
                    ptype = custom.get("type", "openai-compatible")
                    # Map type to known formatter
                    if "anthropic" in ptype:
                        format_provider = "anthropic"
                    else:
            ...
            try:
                ...
                raise PrompdError(f"Failed to parse compiled output: {e}") from e
            ...
            if isinstance(e, PrompdError):
                ...
                raise PrompdError(f"Execution failed: {e}") from e
    ...
    async def _execute_with_provider(
        ...
        try:
            ...
            raise ProviderError(f"Provider '{context.provider}' not available: {e}") from e
        ...
        try:
            ...
            if isinstance(e, ProviderError):
                ...
                raise ProviderError(f"Provider execution failed: {e}") from e
