...
from prompd.models import PrompdFile, PrompdMetadata
from prompd.section_override_processor import SectionInfo, SectionOverrideProcessor
...
class PrompdParser:
    ...
    def parse_content(self, content: str) -> PrompdFile:
        ...
        try:
            ...
            raise ParseError(f"Invalid metadata: {e}") from e
    ...
    def resolve_content_reference(
        ...
        Args:
            reference: Content reference (file path, #section, list of references, or None)
        ...
        # Handle list references by resolving each and concatenating
        if isinstance(reference, list):
            resolved_parts = []
            for ref in reference:
                resolved = self.resolve_content_reference(ref, sections, base_path)
                if resolved:
        ...
        # File reference (./path/to/file.md or ../path/to/file.md)
        elif reference.startswith("./") or reference.startswith("../") or reference.startswith("/"):
            ...
            try:
                ...
                raise ParseError(f"Failed to read referenced file {file_path}: {e}") from e
    ...
    def extract_sections_from_file(self, file_path: Path) -> Dict[str, SectionInfo]:
        ...
        try:
            ...
            raise ParseError(f"Failed to extract sections from {file_path}: {e}") from e
