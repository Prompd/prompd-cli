            ...
            def _ping():
                try:
                    ...
                    if self.registry.registry_info is None:
                        self.console.print(
                            f"[dim]Note: Registry not reachable at "
                            f"{self.registry.registry_url}. "
                            f"Registry features may be offline.[/dim]"
                        )
                except Exception:
                    # Quietly ignore
                    pass
            ...
            try:
                ...
                if self.chat_mode:
                    ...
                    if current_provider:
                        ...
                        self.console.print(
                            f"[green]prompd[/green][dim]"
                            f"({provider_short})[/dim]"
                            f"[yellow]>[/yellow] ",
                            end=""
                        )
                    else:
    ...
    def handle_chat_input_enhanced(self, user_input: str, show_user_panel=True):
        ...
        if intent == "compile":
            ...
            try:
                ...
                if suggestions:
                    self.console.print(
                        f"[yellow]No prompt named "
                        f"'{intent_data['target']}' found. "
                        f"Did you mean: "
                        f"{', '.join(suggestions[:3])}?[/yellow]"
                    )
                else:
                    self.console.print(
                        f"[yellow]No prompt named "
                        f"'{intent_data['target']}' found. "
                        f"Showing available prompts:[/yellow]"
                    )
    ...
    def execute_command(self, command_text: str):
        ...
        try:
            if command == "help":
                ...
                self.console.print(
                    "Type [cyan]help[/cyan] for available "
                    "commands or [cyan]chat[/cyan] to talk naturally"
                )
        ...
        if directories:
            if self.compact_mode:
                ...
                dirs = ", ".join([
                    f"[cyan]{d.name}/[/cyan]"
                    for d in sorted(
                        directories, key=lambda d: d.name.lower()
                    )
                ])
        ...
        if prompd_files:
            if self.compact_mode:
                ...
                files = ", ".join([
                    ...
                    for f in sorted(
                        prompd_files, key=lambda f: f.name.lower()
                    )
                ])
                self.console.print(
                    f"\n[cyan]Prompts ({len(prompd_files)}):"
                    f"[/cyan] {files}"
                )
            else:
                # Standard format
        ...
        if pdpkg_files:
            if self.compact_mode:
                files = ", ".join([
                    f"[blue]{f.name}[/blue]"
                    for f in sorted(
                        pdpkg_files, key=lambda f: f.name.lower()
                    )
                ])
            ...
            if provider_name == "openai":
                ...
                if config.get_api_key("anthropic"):
                    ...
                    self.console.print(
                        ...
                        "Use 'prompd provider add anthropic' "
                        "to set up.[/red]"
                    )
