...
from prompd.models import ExecutionContext, LLMMessage, LLMRequest, LLMResponse, MessageRole
    ...
    async def execute(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            try:
                ...
                error_content = str(e)
            raise ProviderError(f"Custom provider API error: {error_content}") from e
        except httpx.RequestError as e:
            raise ProviderError(f"Request to custom provider failed: {e}") from e
        except json.JSONDecodeError as e:
            raise ProviderError(f"Invalid JSON response from custom provider: {e}") from e
        except Exception as e:
            raise ProviderError(f"Unexpected error with custom provider: {e}") from e
