...
from prompd.commands.config import config  # noqa: E402
            ...
            try:
                ...
                if prompd.metadata and hasattr(prompd.metadata, "inherits") and prompd.metadata.inherits:
                    if hasattr(prompd.metadata, "override") and prompd.metadata.override:
                        ...
                        if verbose:
                            console.print(
                                f"\n[blue]Override Check:[/blue] File inherits from "
                                f"{prompd.metadata.inherits} but has no overrides"
                            )
                else:
                    if verbose:
        ...
        if prompd.sections:
            if not detailed and not show_sections:
                ...
                output += (
                    ...
                    f"{', '.join(all_section_names)} "
                    f"({len(all_section_names)} total)\n"
                )
            else:
    ...
    try:
        ...
        with zipfile.ZipFile(package_path, "r") as zf:
            ...
            try:
                ...
                if verbose:
                    console.print("[dim]Parsed manifest.json[/dim]")
            except KeyError:
...
def _explain_registry_package(package_name: str, registry: Optional[str], detailed: bool, verbose: bool):
    ...
    try:
        ...
        if verbose:
            console.print("[dim]Received package info[/dim]")
    ...
    try:
        ...
        if git:
            ...
            if latest_tag and latest_tag != current_version:
                console.print("[yellow]WARNING[/yellow] Version mismatch:")
...
def _git_commit_and_tag(file_path: Path, version: str, message: str):
    ...
    try:
        from prompd.security import (
            SecurityError,
            validate_git_file_path,
        ...
        raise Exception(f"Git operation failed: {e.stderr.decode()}") from e
...
def _git_diff_versions(file_path: Path, version1: str, version2: str) -> str:
    ...
    try:
        ...
        raise Exception(f"Git diff failed: {e.stderr.decode()}") from e
    ...
    try:
        ...
        for package_name in packages:
            ...
            if global_uninstall:
                ...
                if len(matching) > 1:
                    console.print("[yellow]Multiple versions found. Please specify version:[/yellow]")
                    for p in matching:
...
def publish(package_file: Path, registry: Optional[str], namespace: Optional[str], dry_run: bool):
    ...
    try:
        ...
        if namespace:
            ...
            console.print("[dim]Namespace: none (will use package scope or registry default)[/dim]")
...
def namespace_create(
    ...
    try:
        ...
        if result.get("requiresVerification"):
            console.print("[yellow]Namespace requires verification[/yellow]")
...
def init(path: Path, name: Optional[str], version: str, description: Optional[str], author: Optional[str]):
    ...
    console.print("[green]OK[/green] Created manifest.json")
...
"""
        ...
        console.print("[green]OK[/green] Created example.prmd")
    ...
    console.print("  prompd validate example.prmd")
