        ...
        for section_id, override_path in v.items():
            ...
            if override_path is not None:
                if not isinstance(override_path, str):
                    raise ValueError(
                        f"Override path must be a string or null, "
                        f"got {type(override_path).__name__}: {override_path}"
                    )
                ...
                for char in invalid_chars:
                    if char in override_path:
                        raise ValueError(
                            f"Override path contains invalid character '{char}' "
                            f"for section '{section_id}': {override_path}"
                        )
