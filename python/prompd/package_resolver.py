...
class BasePackageCache:
    ...
    def _validate_zip_contents_secure(self, zip_file: zipfile.ZipFile):
        ...
        for member in zip_file.namelist():
            ...
            try:
                ...
                raise PrompdError(f"Invalid file in package: {member}") from None
            ...
            if not member.endswith("/"):  # Not a directory
                try:
                    ...
                    raise PrompdError(f"Path validation failed for {member}: {e}") from e
    ...
    def _extract_zip_safely(self, zip_file: zipfile.ZipFile, target_dir: Path):
        ...
        for member in zip_file.namelist():
            ...
            try:
                ...
                raise PrompdError(f"Path resolution failed for {member}: {e}") from e
    ...
    def _validate_extracted_package(self, package_dir: Path, package_ref: PackageReference):
        ...
        try:
            ...
            raise PrompdError(f"Invalid {manifest_file.name}: {e}") from e

        # Validate manifest structure
        required_fields = ["name", "version"]
        for req_field in required_fields:
            if req_field not in manifest:
                raise PrompdError(f"Manifest missing required field: {req_field}")
...
class PackageResolver:
    ...
    def _download_package(
        self, registry_url: str, registry: RegistryInfo, package_ref: PackageReference
    ) -> Tuple[bytes, str]:
    ...
    def get_package_manifest(self, package_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            raise PrompdError(f"Invalid package manifest ({manifest_file.name}): {e}") from e
