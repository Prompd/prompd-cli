...
class ConversationalAssistant:
    ...
    def get_ai_response(self, user_input: str, shell_instance=None) -> str:
        ...
        if "app_name" in user_input_lower and "=" not in user_input_lower:
            return (
                "For parameters, try: "
                "'compile test-prompt app_name=\"MyApp\"' or "
                "'compile test-prompt with app_name=\"MyApp\"'"
            )
        ...
        return (
            ...
            "Try asking me to compile, show, list files, "
    ...
    def try_ai_providers(self, user_input: str, shell_instance=None) -> str:
        ...
        try:
            ...
            if not assistant_prompt.exists():
                ...
                if shell_instance:
                    shell_instance.console.print(
                        f"[dim]Debug: Assistant prompt file not found at {assistant_prompt}[/dim]"
                    )
    ...
    def try_ai_providers_fallback(self, user_input: str, shell_instance=None) -> str:
        ...
        try:
            ...
            from .models import LLMMessage, LLMRequest, MessageRole
    ...
    def handle_complex_file_operations(self, user_input: str, shell_instance) -> str:
        ...
        complex_pattern = (
            r"make.*?folder.*?(?:called|named)\s+(.+?)\s+and\s+move\s+"
            r"(?:the\s+)?(.+?)\s+(?:to|into?)\s+(?:the\s+)?(?:new\s+)?folder"
        )
...
# System
You write engaging, original stories tailored to the request.
Keep the story consistent and avoid overlong exposition.

# User
Write a {{length}} {{genre}} story featuring {{protagonist}} as the main character.
Keep it self-contained with a beginning, middle, and end.
"""
        else:
            # Generic template
            template = f"""---
...
class PrompdShell:
            ...
            def _ping():
                try:
                    ...
                    if self.registry.registry_info is None:
                        ...
                        self.console.print(
                            f"[dim]Note: Registry not reachable at {self.registry.registry_url}."
                            " Registry features may be offline.[/dim]"
                        )
                except Exception:
                    # Quietly ignore
                    pass
    ...
    def parse_ai_response(self, response: str) -> tuple:
        ...
        try:
            ...
            response_data.get("showFiles", False)
    ...
    def handle_chat_input_enhanced(self, user_input: str, show_user_panel=True):
        ...
        if intent == "compile":
            ...
            try:
                ...
                if (
                    "how to" in target_lower
                    or "how do i" in target_lower
                    or "make" in target_lower
                    or "create" in target_lower
                ) and "prompd" in target_lower:
                ...
                if suggestions:
                    self.console.print(
                        f"[yellow]No prompt named '{intent_data['target']}' found."
                        f" Did you mean: {', '.join(suggestions[:3])}?[/yellow]"
                    )
                else:
                    self.console.print(
                        f"[yellow]No prompt named '{intent_data['target']}' found."
                        " Showing available prompts:[/yellow]"
                    )
    ...
    def interactive_compile(self, args: List[str]):
        ...
        if path.exists():
            ...
            try:
                ...
                self.console.print("[green]Compilation completed![/green]")
    ...
    def switch_provider(self, provider_name: str):
        ...
        try:
            ...
            if provider_name == "openai":
                if config.get_api_key("openai"):
                    self.console.print("[green]Switched to OpenAI provider[/green]")
                    # Could set a preference in config here
                else:
                    self.console.print("[red]OpenAI not configured. Use 'prompd provider add openai' to set up.[/red]")
                ...
                if self.check_ollama_available():
                    self.console.print("[green]Switched to Ollama provider[/green]")
                    self.console.print("[dim]Note: Ollama integration is experimental[/dim]")
                else:
                    self.console.print("[red]Ollama not available. Install Ollama and ensure it's running.[/red]")
    ...
    def interactive_login(self, args: List[str]):
        ...
        if not args or (len(args) >= 2 and args[0] == "--token"):
            ...
            args[1]
            self.console.print("[cyan]Logging in with token...[/cyan]")
