...
class DependencyResolutionStage(CompilerStage):
    ...
    def process(self, context: CompilationContext) -> None:
        ...
        if hasattr(context.metadata, "inherits") and context.metadata.inherits:
            ...
            # e.g., "@pkg/templates/file.prmd" -> "@scope/package@version/templates/file.prmd"
            ...
            is_local = inherits_ref.startswith(("./", "../", "/"))
            is_non_pkg_prmd = not inherits_ref.startswith("@") and ".prmd" in inherits_ref
                ...
                if isinstance(package_info, dict):
                    ...
                    if prefix and package_path:
                        ...
                        def replace_ref(match, _pkg_path=package_path, _prefix=prefix):
                            ...
                            possible_files = [
                                Path(_pkg_path) / resource_path,
                                Path(_pkg_path) / f"{resource_path}.prmd",
                                Path(_pkg_path) / f"{resource_path}.md",
                                Path(_pkg_path) / f"{resource_path}.txt",
                            ]
                                    ...
                                    try:
                                        ...
                                        if file_path.suffix == ".prmd":
                                            ...
                                            try:
                                                ...
                                                return parsed.content or f"# Content from @{_prefix}/{resource_path}"
                                            except Exception:
                                        ...
                                        return f"[Error loading @{_prefix}/{resource_path}]"

                            # File not found
                            if context.verbose:
                                print(f"Warning: Could not find @{_prefix}/{resource_path} in {_pkg_path}")
                            return f"[Not found: @{_prefix}/{resource_path}]"
                        ...
                        if overrides:
                            ...
                            try:
                                ...
                                context.warnings.append(
                                    f"Section override processing failed, using simple inheritance: {e}"
                                )
                                if parent_data.content:
                                    if context.content:
                    ...
                    for section_id, override_path in overrides.items():
                        if section_id in current_sections:
                            if override_path is None:
                                ...
                                try:
                                    override_content = self.section_processor.load_override_content(
    ...
    def execute(self, source: Union[str, Path], **options) -> CompilationContext:
        ...
        if isinstance(source, str) and source.startswith("@"):
            ...
            try:
                ...
                raise PrompdError(f"Failed to resolve package {source}: {e}") from e
        else:
