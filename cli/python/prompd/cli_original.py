...
def list_cache(global_only: bool, local_only: bool):
    ...
    try:
        ...
        if packages_dict['local']:
            console.print("\n[bold cyan]Local Project Cache (./.prmd/cache/):[/bold cyan]")
...
def dependencies(package: Optional[str], tree: bool, conflicts: bool, dev: bool, peer: bool, depth: int):
    ...
    if not package:
        config_file = Path.cwd() / '.prmd' / 'config.yaml'
        if config_file.exists():
            ...
            console.print("[red]No package specified and no .prmd/config.yaml found[/red]")
            sys.exit(1)
...
def install_dependencies(package: str, save: bool, save_dev: bool, target: Optional[str], parallel: bool):
    ...
    try:
        ...
        target_dir = Path(target) if target else Path.cwd() / '.prmd' / 'packages'
